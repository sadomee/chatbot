...
# 2. القاموس الكامل (كل النصوص الأصلية كاملة)
t = {
    ...
    * Seek urgent medical care if symptoms worsen.
    ...
    "t1_p": "لا تبدئي الصيام دون استشارة طبيبك لتعديل جرعات الأنسولين أو الأدوية." if is_ar else "Don't start fasting without consulting your doctor.",
    "t2_h": "توازن الوجبات:" if is_ar else "Balanced Meals:",
    "t2_p": "ابدئي إفطارك بالتمر والماء، واجعلي وجبة السحور غنية بالألياف والبروتين." if is_ar else "Start Iftar with dates and water.",
    "t3_h": "الحماية من الجفاف:" if is_ar else "Dehydration Protection:",
    "t3_p": "اشربي لترين من الماء على الأقل في الفترة ما بين الإفطار والسحور." if is_ar else "Drink at least two liters of water.",
    "t4_h": "الفحص الدوري:" if is_ar else "Regular Testing:",
    "t4_p": "قومي بقياس مستوى السكر في الدم 4 مرات يومياً على الأقل خلال الصيام." if is_ar else "Measure your blood sugar 4 times a day.",
    "footer_title": "رمضان بصحة" if is_ar else "Healthy Ramadan",
    "footer_p": "مساعدكِ الطبي الذكي المتخصص في رفع الوعي الصحي لمرضى السكري خلال الشهر الفضيل." if is_ar else "Your smart medical assistant for diabetics.",
...
st.markdown(f"""
    ...
    * {{ font-family: 'Cairo', sans-serif !important; direction: {"rtl" if is_ar else "ltr"}; text-align: {"right" if is_ar else "left"}; }}
    ...
    .about-box h3 {{ color: #1b4332; }}
    .about-box p {{ color: #4b5563; line-height: 1.8; font-size: 1.1rem; }}
    .card-container {{ background: white; padding: 25px; border-radius: 25px; border: 1px solid #eee; text-align: center; margin-bottom: 20px; }}
...
if st.session_state.page == 'home':
    ...
    with col_title: st.markdown(f'<h2 style="color:#1b4332;">{t["title"]}</h2>', unsafe_allow_html=True)
    with col_emergency:
        # هنا يبقى اللون أحمر لأنه للطوارئ
        if st.button(t["emergency_btn"], type="secondary", key="em"): st.error(t["emergency_msg"])
    ...
    with col_start[1]:
        if st.button(t["start_btn"], type="primary", key="start"):
...
# --- صفحة الشات ---
    ...
    clicked_q = None
    with sq1: 
        # تم تغيير النوع هنا إلى primary ليصبح لونها أخضر
    ...
    if prompt:
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[{"role": "system", "content": "You are a medical assistant."}] + st.session_state.messages
            )
